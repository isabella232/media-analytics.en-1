...
uuid: ee2b38cb-e244-4e5d-94a8-4847794a24a6
index: y
internal: n
snippet: y
translate: y
...
* **Mobile Static** 
  ...
  Mobile static measurement relies on Adobe’s Mobile Services SDK, so by default, the two opt-outs are aligned. To offer an opt-out, you need to complete the additional steps below.

* **Mobile Video** 

  Mobile video measurement relies on Adobe’s Mobile Services SDK and Nielsen’s SDK.
...
* **Mobile Static** 
  ...
  If you separate opt-out for mobile static, there are a few required steps:
...
* **Mobile Video** 
  ...
  Because the alignment of the opt-out requires extra steps for the app developer, separating the two opt-outs for mobile video involves not taking those extra steps.
